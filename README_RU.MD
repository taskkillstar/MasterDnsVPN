...
Клиент и сервер поддерживают следующие флаги командной строки (все параметры из `client_config.toml` также можно переопределять через флаги CLI):

| Параметр | Алиас | Для кого | Описание |
| :--- | :--- | :--- | :--- |
| `-config` | `-c` | Оба | Путь к файлу конфигурации (по умолчанию: `client_config.toml` / `server_config.toml`) |
| `-resolvers` | `-r` | Клиент | Переопределение пути к файлу резолверов (по умолчанию: `client_resolvers.txt`) |
| `-domains` | `-d` | Клиент | Переопределение доменов туннеля через запятую |
| `-encryption-key` | `-k` | Клиент | Переопределение ключа шифрования |
| `-log` | `-l` | Оба | Опциональный путь к лог-файлу |
| `-version` | `-v` | Оба | Вывод версии и выход |
| `-help` | `-h` | Оба | Показать справку и все доступные флаги переопределения |
...
- `IP` (IPv4 или `[IPv6]`)
- `IP:PORT` (IPv4 или `[IPv6]:PORT`)
- `CIDR` (например, `192.168.1.0/24`)
- `CIDR:PORT` (например, `192.168.1.0/24:5300`)
...
# --- Пользовательские резолверы и подсети ---
# Cloudflare DNS
1.1.1.1

# Локальная подсеть провайдера
192.168.10.0/24:5300
```

> 💡 **Умный быстрый старт (Smart Warm-Start):** Клиент сохраняет точный порядок строк из `client_resolvers.txt`. При включенном `AUTO_SAVE_RANKED_RESOLVERS = true` клиент автоматически определяет и сохраняет проверенные самые быстрые резолверы вверху файла внутри управляемого заголовка. Все пользовательские комментарии, форматирование и подсети ниже блока остаются нетронутыми. FastStart использует эти верхние записи для подключения менее чем за 150 мс при последующих запусках.
...
| Параметр | Пример значения | Значения | Полное описание |
| :--- | :--- | :--- | :--- |
| `RESOLVER_BALANCING_STRATEGY` | `6` | `0..8` | Определяет способ выбора резолверов.<br>`0/2` = Round Robin (по кругу), `1` = случайный выбор, `3` = минимальные потери, `4` = наименьшая задержка, `5` = гибридная оценка, `6` = сначала потери, затем задержка (по умолчанию), `7` = случайный выбор среди лучших по потерям, `8` = циклический выбор среди лучших по потерям.<br>Гибридный режим использует взвешенную комбинированную оценку. Режим "сначала потери, затем задержка" сначала отбирает по уровню потерь, затем предпочитает меньшую задержку внутри этой группы и распределяет нагрузку между близкими по характеристикам кандидатами. Режим «top-random» выбирает случайно из лучшей группы по потерям, чтобы нагрузка не концентрировалась на одном резолвере. Режим "top-round-robin" последовательно перебирает ту же лучшую группу по потерям с детерминированной ротацией. |
| `PACKET_DUPLICATION_COUNT` | `2` | Допустимое значение ограничено в коде | Количество дублирования исходящих пакетов в обычном режиме.<br>Более высокие значения увеличивают сетевые накладные расходы, но повышают устойчивость на нестабильных каналах.
| `SETUP_PACKET_DUPLICATION_COUNT` | `2` | Допустимое значение ограничено в коде | Аналогично `PACKET_DUPLICATION_COUNT`, но используется для пакетов, чувствительных к настройкам, таких как создание потока и другие критические управляющие события. |
...
| `RECHECK_INACTIVE_SERVERS_ENABLED` | `true` | `true/false` | Включает фоновую повторную проверку резолверов, которые отключены или считаются неработоспособными.<br>Также активирует **FastStart**: подключение сразу после валидации `RX_TX_WORKERS` серверов с допроверкой остальных в фоне.<br>Если отключено, резолвер, ставший недоступным, останется отключённым до перезапуска. |
| `AUTO_SAVE_RANKED_RESOLVERS` | `true` | `true/false` | Автоматически сохраняет самые эффективные и быстрые резолверы в управляемый заголовок файла `client_resolvers.txt`.<br>При следующем запуске FastStart мгновенно использует проверенные быстрые резолверы без необходимости сканировать весь список с нуля.<br>Сохраняет все пользовательские комментарии и подсети ниже блока. |
...
### 3.4.7) ⚡ Микро-бёрст тестирование пропускной способности и квалификация скорости

| Параметр | Пример значения | Значения | Полное описание |
| :--- | :--- | :--- | :--- |
| `MICRO_BURST_TEST_ENABLED` | `true` | `true/false` | Запускает конвейерное микро-бёрст тестирование после MTU-тестов для выявления rate-limiting и замера реальной пропускной способности. |
| `MICRO_BURST_PACKET_COUNT` | `6` | `2..32` | Количество последовательных запросов в пачке, отправляемых каждому резолверу при тестировании. |
| `MICRO_BURST_PARALLELISM` | `8` | `1..128` | Параллелизм бёрст-тестирования. Рекомендуется `6-8` для Android/мобильных устройств, `16-32` для ПК/серверов. |
| `MAX_ACTIVE_RESOLVERS` | `15` | `>=0` | Максимальное число лучших резолверов, переводимых в активный пул (`0` = без ограничений). Остальные остаются в резерве (standby). |
| `MAX_BURST_LOSS_RATIO` | `0.20` | `0.0..1.0` | Максимально допустимый процент потерь при бёрст-тесте (`0.20` = до 20% потерь). |
| `MIN_BURST_THROUGHPUT_KBPS` | `0.0` | `>=0.0` | Минимальный порог скорости в КБ/с для допуска резолвера (`0.0` = отключено). |
| `RECHECK_BURST_TEST_ENABLED` | `true` | `true/false` | Проверять неактивные резолверы микро-бёрстом перед возвращением в рабочий пул при фоновой перепроверке. |
| `RECHECK_BURST_PACKET_COUNT` | `4` | `2..32` | Количество запросов в бёрсте при фоновой проверке неактивных резолверов. |

### 3.4.8) 📊 Мониторинг производительности резолверов в реальном времени

| Параметр | Пример значения | Значения | Полное описание |
| :--- | :--- | :--- | :--- |
| `RUNTIME_STATS_INTERVAL_SECONDS` | `60.0` | `1.0..86400.0` (или `0` = выкл) | Интервал в секундах между выводом сводной таблицы в консоль при наличии трафика.<br>Значение `0` отключает периодический вывод.<br>Нажатие клавиши `'s'` + Enter (или просто Enter) в консоли в любой момент мгновенно выводит актуальную таблицу с метриками. |

### 3.4.9) 🧵 Потоки, очереди и таймеры выполнения
...
### 3.4.10) 📡 Пинг / поддержание соединения (Ping / Keepalive)
